...
bool PongMode::handle_event(SDL_Event const &evt, glm::uvec2 const &window_size)
{
	...
	if (evt.type == SDL_MOUSEMOTION)
	{
		...
		down_paddle.x = (clip_to_court * glm::vec3(clip_mouse, 1.0f)).x;
		up_paddle.x = (clip_to_court * glm::vec3(clip_mouse, 1.0f)).x;
	}
...
*/
	...
	float speed_multiplier = 4.0f * std::pow(1.0f, (left_score + right_score) / 4.0f);
	...
	auto paddle2_vs_ball = [this](glm::vec2 const &paddle)
	{
		//compute area of overlap:
		glm::vec2 min = glm::max(paddle - paddle2_radius, ball - ball_radius);
		glm::vec2 max = glm::min(paddle + paddle2_radius, ball + ball_radius);
		...
		if (max.x - min.x > max.y - min.y)
		{
			...
			if (ball.y > paddle.y)
			{
				ball.y = paddle.y + paddle2_radius.y + ball_radius.y;
				ball_velocity.y = std::abs(ball_velocity.y);
			}
			else
			{
		...
		else
		{
			...
			if (ball.x > paddle.x)
			{
				ball.x = paddle.x + paddle2_radius.x + ball_radius.x;
				ball_velocity.x = std::abs(ball_velocity.x);
			}
			else
			{
			...
			//warp y velocity based on offset from paddle center:
			float vel = (ball.y - paddle.y) / (paddle2_radius.y + ball_radius.y);
			ball_velocity.y = glm::mix(ball_velocity.y, vel, 0.75f);
		}
	};
	paddle2_vs_ball(down_paddle);
	paddle2_vs_ball(up_paddle);

	auto obstacle_vs_ball = [this](glm::vec2 const &paddle, uint32_t bounce, uint32_t obj_color)
	{
		//compute area of overlap:
		glm::vec2 min = glm::max(paddle - paddle_radius, ball - ball_radius);
		glm::vec2 max = glm::min(paddle + paddle_radius, ball + ball_radius);
		...
		if (max.x - min.x > max.y - min.y)
		{
			...
			if (ball.y > paddle.y)
			{
				ball.y = paddle.y + obstacle_radius.y + ball_radius.y;
				ball_velocity.y = std::abs(ball_velocity.y);
				if ((bounce % 3 == obj_color) && (obj_color == 0))
				{
					left_score += 3;
				}
				else if ((bounce % 3 == obj_color) && (obj_color == 1))
				{
					left_score += 2;
				}
				else if ((bounce % 3 == obj_color) && (obj_color == 2))
				{
					left_score += 1;
				}
				bounce_time += 1;
		...
		else
		{
			...
			if (ball.x > paddle.x)
			{
				ball.x = paddle.x + obstacle_radius.x + ball_radius.x;
				ball_velocity.x = std::abs(ball_velocity.x);
				if ((bounce % 3 == obj_color) && (obj_color == 0))
				{
					left_score += 3;
			...
			//warp y velocity based on offset from paddle center:
			float vel = (ball.y - paddle.y) / (obstacle_radius.y + ball_radius.y);
			ball_velocity.y = glm::mix(ball_velocity.y, vel, 0.75f);
		}
	};

	obstacle_vs_ball(obstacleR1, bounce_time, 0);
	obstacle_vs_ball(obstacleR2, bounce_time, 1);
	obstacle_vs_ball(obstacleR3, bounce_time, 1);
	obstacle_vs_ball(obstacleR4, bounce_time, 2);
	obstacle_vs_ball(obstacleR5, bounce_time, 2);
	obstacle_vs_ball(obstacleR6, bounce_time, 2);
	obstacle_vs_ball(obstacleL1, bounce_time, 0);
	obstacle_vs_ball(obstacleL2, bounce_time, 1);
	obstacle_vs_ball(obstacleL3, bounce_time, 1);
	obstacle_vs_ball(obstacleL4, bounce_time, 2);
	obstacle_vs_ball(obstacleL5, bounce_time, 2);
	obstacle_vs_ball(obstacleL6, bounce_time, 2);
...
#define HEX_TO_U8VEC4(HX) (glm::u8vec4((HX >> 24) & 0xff, (HX >> 16) & 0xff, (HX >> 8) & 0xff, (HX)&0xff))
	...
	const glm::u8vec4 paddle_color = HEX_TO_U8VEC4(0xb6edf2ff);
	const glm::u8vec4 red_color = HEX_TO_U8VEC4(0xed7e7eff);
	const glm::u8vec4 yellow_color = HEX_TO_U8VEC4(0xedd97eff);
	const glm::u8vec4 green_color = HEX_TO_U8VEC4(0x9aed7eff);
	const glm::u8vec4 blue_color = HEX_TO_U8VEC4(0x7ee7edff);

	const std::vector<glm::u8vec4> trail_colors = {
		HEX_TO_U8VEC4(0xed7e7e88),
		HEX_TO_U8VEC4(0xedd97e88),
		HEX_TO_U8VEC4(0x9aed7e88),
		HEX_TO_U8VEC4(0x7ee7ed88),
		HEX_TO_U8VEC4(0x7e7eedff)};
#undef HEX_TO_U8VEC4
	...
	*/
	...
	//paddles:
	draw_rectangle(left_paddle, paddle_radius, paddle_color);
	draw_rectangle(right_paddle, paddle_radius, paddle_color);
	draw_rectangle(down_paddle, paddle2_radius, paddle_color);
	draw_rectangle(up_paddle, paddle2_radius, paddle_color);

	//obstacles:
	draw_rectangle(obstacleL1, obstacle_radius, red_color);
	draw_rectangle(obstacleR1, obstacle_radius, red_color);
	draw_rectangle(obstacleL2, obstacle_radius, yellow_color);
	draw_rectangle(obstacleL3, obstacle_radius, yellow_color);
	draw_rectangle(obstacleR2, obstacle_radius, yellow_color);
	draw_rectangle(obstacleR3, obstacle_radius, yellow_color);
	draw_rectangle(obstacleL4, obstacle_radius, green_color);
	draw_rectangle(obstacleL5, obstacle_radius, green_color);
	draw_rectangle(obstacleL6, obstacle_radius, green_color);
	draw_rectangle(obstacleR4, obstacle_radius, green_color);
	draw_rectangle(obstacleR5, obstacle_radius, green_color);
	draw_rectangle(obstacleR6, obstacle_radius, green_color);

	//ball:
	if (bounce_time % 3 == 0)
	{
		draw_rectangle(ball, ball_radius, red_color);
	}
	else if (bounce_time % 3 == 1)
	{
		draw_rectangle(ball, ball_radius, yellow_color);
	}
	else if (bounce_time % 3 == 2)
	{
		draw_rectangle(ball, ball_radius, green_color);
	}
	else
	{
		draw_rectangle(ball, ball_radius, blue_color);
	}
